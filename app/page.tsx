...
{/* PROMO GALLERY */}
<section className="promo-section" id="gallery">
...
{/* TESTIMONIALS */}
<section className="testimonials-section relative overflow-hidden bg-gradient-to-b from-white via-[var(--cream)] to-white !py-16 md:!py-20" id="testimonials">
  ...
  <div className="testimonials-grid relative z-10 max-w-[1360px] mx-auto px-4 mt-10">
    {siteConfig.testimonials.slice(0, 6).map((item, index) => {
      ...
      return (
        <div 
          key={item.name} 
          className={`relative flex flex-col justify-between rounded-[28px] border border-[var(--lavender-mid)]/50 bg-gradient-to-br from-white via-white to-[var(--lavender-bg)]/20 p-6 shadow-[0_12px_40px_rgba(91,45,142,0.03)] transition-all duration-300 hover:-translate-y-2 hover:scale-[1.01] hover:border-[var(--purple-light)] hover:shadow-[0_24px_56px_rgba(123,75,184,0.12)] reveal${delayClass} visible`}
        >
          {/* Decorative Top-Right Quote Mark */}
          <span className="absolute top-2 right-4 select-none font-serif text-[84px] leading-none text-[var(--purple-light)]/15">&ldquo;</span>
          
          <div className="relative z-10">
            {/* Rating pill */}
            <div className="flex items-center gap-1 rounded-full bg-amber-500/10 px-2.5 py-1 text-amber-600 w-fit text-[11px] font-extrabold mb-4 shadow-[inset_0_1px_0_rgba(251,191,36,0.1)]">
              {"★".repeat(item.rating)}
            </div>
            
            <p className="font-body-main text-[14px] leading-[1.75] text-[var(--grey)] italic mb-6">
              "{item.quote}"
            </p>
          </div>
          
          <div className="relative z-10 flex items-center gap-3.5 pt-4 border-t border-[var(--lavender-mid)]/35">
            <img 
              src={item.image} 
              alt={item.alt} 
...
<section className="cta-section">
  ...
  <p>Book your personalised consultation at Ariix Hair and Skin Clinic, Wagholi, Pune today.</p>
